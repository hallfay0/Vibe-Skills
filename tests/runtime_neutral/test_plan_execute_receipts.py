...
class PlanExecuteReceiptTests(unittest.TestCase):
    def test_plan_execute_receipt_and_proof_manifest_expose_current_execution_fields(self) -> None:
        ...
        text = script_path.read_text(encoding="utf-8")
        proof_block = text.split("$proofManifest = [pscustomobject]@{", 1)[1].split("$proofManifestPath =", 1)[0]
        receipt_block = text.split("$receipt = [pscustomobject]@{", 1)[1].split("$receiptPath =", 1)[0]
