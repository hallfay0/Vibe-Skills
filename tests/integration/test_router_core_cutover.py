...
def test_router_contract_runtime_keeps_support_and_selection_helpers_but_owns_route_presentation() -> None:
    ...
    assert 'from .router_contract_selection import (' in runtime
    assert 'get_pack_default_candidate,' in runtime
    assert 'get_pack_skill_candidates,' in runtime
    assert 'select_pack_candidate,' in runtime
