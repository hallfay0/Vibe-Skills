...
4. Install into one shared root by default, not an isolated demo folder:
   - every host shares `~/.agents`; on Windows that means `%USERPROFILE%\\.agents`
   - `codex`: default to `~/.agents` so the same install stays reusable across hosts; set `VIBE_AGENTS_HOME` only when I explicitly ask for a different shared root
...
7. During installation, keep the public contract narrow: host plugins, providers, and online enhancement remain host-managed.
8. `$vibe` or `/vibe` is governed runtime entry only and does not prove host plugins, providers, or online enhancement are complete.
9. Repo templates, manifests, examples, sidecars, or commands on PATH are not enough to prove online-ready.
10. End with a concise final install report that separates: `installed locally`, `vibe host-ready`, `online-ready`, commands executed, and manual follow-up.
```
