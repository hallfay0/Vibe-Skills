...
> Historical / Retired Note: This document discusses retired or cleanup-context routing terminology. The current truth chain is `agent_skill_organization -> module-work-plan.json -> agent-execution-handoff.json -> module-execution.json`; old terms here are historical only and are not current runtime states.
