...
现在公开主叙事更简单：已安装的本地 skill 根目录是唯一专家来源。Codex 会先扫 `~/.agents/skills`，再扫 `~/.codex/skills`；Claude Code 会扫 `~/.claude/skills`。如果同一个 skill id 出现多次，先被扫描到的根目录生效。真正记录这次运行实际绑定了什么的，仍然是 `work_binding`。

在当前运行时边界里，Python 负责最终 truth artifacts、canonical validation、任务语义、`work_binding`、专家决策真相和结构化运行结果。PowerShell 仍然承担阶段编排、环境准备、脚本桥接、宿主收据、shell 原生检查和叶子执行；这版不把它描述成已经只是薄壳。
...
| **local-skill-only 参考面** | kernel 只把已安装的本地 skill 根目录当作唯一 specialist 参考面；一个 skill 必须有可读取的 `SKILL.md`，才可能被选中。 |
...
> 现在对外的主叙事更收敛，也更实用：已安装的本地 skill 根目录是唯一 specialist 参考面，重复 skill id 按扫描顺序保留第一个可用入口，而 `work_binding` 继续作为内核运行时第一真相面。这描述的是下一步架构方向，不是“最终架构已经完成”的宣称。
...
VibeSkills 好上手，是因为 `vibe` 接管了流程。你给出目标，harness 把目标变成有边界的工作模型，扫描已安装的本地 skill 根目录，只在真正需要的地方绑定有真实 `SKILL.md` 的 Skills，检查结果，并把上下文留给下一次。
...
| `local-skill-only reference plane` | kernel 只把已安装的本地 skill 根目录当作 specialist 来源；如果有重复项，按扫描顺序保留第一个可用入口。 |
...
VibeSkills 属于第二种。它把工作流程、专家 Skills、验证和工作区记忆，封装成一个可迁移的工作内核入口。更重要的是，它把扩展面收敛到已安装的本地 skill 根目录：随着 skill 变多，同一个 `vibe` 入口仍然能让任务有阶段、有检查、能继续，而不用重新长出中心技能目录。
...
| **本地安装扩展** | 已安装的本地 skill 根目录，是扩展这套流程的主要方式；一个 skill 需要真实 `SKILL.md` 才能被绑定。 |
...
这张表的作用是让你快速判断“这个内核大概能不能组织我的工作”，而不是把 README 继续写成技能大全。如果你的任务大致落在这些行里，内核就该能组织起来；如果不在，正常扩展路径仍然是声明的本地 skill 根目录，而不是继续长一个中心技能目录。
...
|                               🧩 能力材料                               |                               ✅ 工作闭环                               |                               ⚖️ 边界纪律                                |
| :---------------------------------------------------------------------: | :--------------------------------------------------------------------: | :----------------------------------------------------------------------: |
| <h2>可组合</h2>已安装的本地 Skills<br/>真实 SKILL.md 才能绑定 | <h2>可落地</h2>目标会变成有边界的工作<br/>再走向测试、检查和产物 | <h2>清边界</h2>小内核和清楚的编排边界<br/>让扩展成本低于路由手术 |
...
现在安装器只做一件事：把当前仓库里的 `vibe` skill 放到一个 skills 目录下。

默认目录是 `~/.agents/skills`。你也可以显式安装到 `~/.codex/skills` 或 `~/.claude/skills`。
...
安装器只写 `<SkillsDir>/vibe`，收据在 `<SkillsDir>/vibe/.vibeskills/install-receipt.json`。它不会改 Codex、Claude、Agents 的设置，不会写入系统提示词，也不会生成多宿主 wrapper。

- 安装入口：`install.ps1 -SkillsDir <skills-dir>`
- 检查入口：`check.ps1 -SkillsDir <skills-dir>`
- 更新入口：`update.ps1 -SkillsDir <skills-dir>`
...
- 📄 [OpenClaw 宿主说明](docs/cold-start-install-paths.md)
- 📄 [OpenCode 宿主说明](docs/cold-start-install-paths.md)
