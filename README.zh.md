...
- **自动推荐任务级别。** VibeSkills 会根据任务范围、步骤数量、依赖关系和是否适合
  同时推进，推荐 `L` 或 `XL`，用户也可以自己选择。`L` 适合多步骤但规模可控的
  任务，会在拆分后按顺序推进，过程更简单、开销较低。`XL` 适合包含多个相对独立
  部分的大任务，会拆得更细，在互不影响时安排最多两项工作同时进行，并增加协调
  和结果汇总。
