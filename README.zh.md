...
| `local-skill-only reference plane` | kernel 只把宿主声明或用户自管的本地 skill 文件夹当作 specialist 来源；如果有重复项，本地用户自管 override 会压过后续重复条目。 |
...
3. 选版本：默认推荐 `minimal`。`minimal` 和 `full` 都只保证 `vibe` / `vibe-upgrade` 公开入口可用；specialist 能力来自你当前宿主能扫描到的本地 skills。
4. 打开安装入口：
...
- 想获得默认的 work-first 内核，让宿主声明或用户自管的 skill 文件夹成为唯一 specialist 来源，并且在重复 skill id 时仍由本地用户自管 override 先赢，就选 `minimal`
- 只有你需要验证 full profile 的安装边界时，再选 `full`；它不会额外启用内置 specialist corpus
- 如果你想一眼看清当前边界，可以直接看 [bundled skill 保留矩阵](docs/governance/bundled-skill-retention-matrix.md)。这里会把历史参考语料、当前安装边界，以及为什么“仓库里曾经有目录”不等于“运行时会调用它”写清楚。
...
如果宿主还声明了额外的用户自管 skill 文件夹，kernel 会把这些本地根当作 specialist 组织的唯一来源；如果出现重复 skill id，本地用户自管 override 仍然先赢。按当前打包合同，`minimal` 和 `full` 都只保证 `vibe` controller 与公开 wrapper 可用，specialist 能力来自当前宿主能扫描到、能打开 `SKILL.md` 的本地 skills。历史 bundled 语料不得作为默认公开扩展面或执行入口。
